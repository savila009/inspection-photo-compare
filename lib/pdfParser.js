...
const MOVE_IN_LABELS =
  /move[\s-]?in|check[\s-]?in|initial\s+inspection|pre[\s-]?occupancy|entry\s+inspection|move[\s-]?in\s+inspection|tenant\s+move[\s-]?in|start\s+of\s+tenancy|lease\s+commencement/i;
const MOVE_OUT_LABELS =
  /move[\s-]?out|check[\s-]?out|final\s+inspection|exit\s+inspection|move[\s-]?out\s+inspection|vacate|vacancy\s+inspection|departure\s+inspection|end\s+of\s+tenancy|lease\s+termination/i;
const GENERIC_INSPECTION_LABELS =
  /inspection\s+date|date\s+of\s+inspection|report\s+date|completed\s+on|conducted\s+on|inspected\s+on|walk[\s-]?through\s+date|condition\s+report\s+date|inspection\s+performed|date\s+of\s+walk[\s-]?through/i;

const SIDE_DATE_LINE = {
  moveIn: [
    /move[\s-]?in\s*(?:inspection)?\s*date\s*[:\s\-]+(.{4,40})/i,
    /check[\s-]?in\s*(?:inspection)?\s*date\s*[:\s\-]+(.{4,40})/i,
    /initial\s+inspection\s*date\s*[:\s\-]+(.{4,40})/i,
    /move[\s-]?in\s*[:\s\-]+(.{4,40})/i,
  ],
  moveOut: [
    /move[\s-]?out\s*(?:inspection)?\s*date\s*[:\s\-]+(.{4,40})/i,
    /check[\s-]?out\s*(?:inspection)?\s*date\s*[:\s\-]+(.{4,40})/i,
    /final\s+inspection\s*date\s*[:\s\-]+(.{4,40})/i,
    /move[\s-]?out\s*[:\s\-]+(.{4,40})/i,
  ],
};
...
export async function parseInspectionPdf(file, side, onProgress) {
  ...
  for (let pageNum = 1; pageNum <= pageCount; pageNum += 1) {
    ...
    if (isTextOnlyPage(pageText, pagePhotos)) {
      onProgress?.(`Skipping text-only page ${pageNum}…`);
      continue;
    }
    ...
    pagePhotos.forEach((photo, index) => {
      const role = classifyPhotoRole(pageText, photo.file.name, photo.width, photo.height);
      if (role === PHOTO_ROLE.REFERENCE) {
        return;
      }
      ...
      extractedPhotos.push({
        ...
        photoRole: role,
        is3D: role === PHOTO_ROLE.PRIMARY || is3DPhoto(pageText, photo.file.name),
      });
    });
  }

  if (!extractedPhotos.length) {
    onProgress?.("No room photos found — checking pages for photo fallback…");
    for (let pageNum = 1; pageNum <= pageCount; pageNum += 1) {
      const pageText = pageTexts[pageNum - 1] || "";
      if (isTextOnlyPage(pageText, [{ width: 800, height: 600 }])) {
        continue;
      }
      ...
      const role = classifyPhotoRole(pageText, fallbackFile.name, 800, 600);
      if (role === PHOTO_ROLE.REFERENCE) {
        continue;
      }
      ...
      extractedPhotos.push({
        ...
        photoRole: role,
        is3D: is3DPhoto(pageText, fallbackFile.name),
        width: 800,
        height: 600,
      });
    }
  ...
  const coverText = normalizeText(pageTexts.slice(0, 3).join("\n\n"));
  const metadata = await pdf.getMetadata().catch(() => null);
  const metadataDate = parsePdfMetadataDate(metadata?.info);
  const dateResult = parseInspectionDate(fullText, side, metadataDate, coverText);
 ...
 */
export function parseInspectionDate(text, side, metadataDate = null, coverText = "") {
  const normalized = normalizeText(text);
  const cover = normalizeText(coverText || normalized.slice(0, 2500));
  const candidates = [];

  collectSideDateLines(cover, side, candidates, 1);
  collectSideDateLines(normalized, side, candidates, 0.95);
  collectLabeledDates(cover, side, candidates, 1);
  collectLabeledDates(normalized, side, candidates, 0.92);
  collectLabeledDates(normalized, side === "moveIn" ? "moveOut" : "moveIn", candidates, 0.2);
  collectGenericInspectionDates(cover, candidates, 0.88);
  collectGenericInspectionDates(normalized, candidates, 0.75);
  collectInlineDates(cover, candidates, 0.35);
...
function collectSideDateLines(text, side, candidates, confidenceMultiplier = 1) {
  const patterns = SIDE_DATE_LINE[side] || [];
  for (const pattern of patterns) {
    const regex = new RegExp(pattern.source, "gi");
    for (const match of text.matchAll(regex)) {
      const parsed = parseFirstDateInString(match[1]);
      if (parsed) {
        candidates.push({
          ...parsed,
          source: side === "moveIn" ? "Move-in date line in PDF" : "Move-out date line in PDF",
          confidence: 0.98 * confidenceMultiplier,
        });
      }
    }
  }
...
function collectGenericInspectionDates(text, candidates, confidenceMultiplier = 1) {
  const lines = text.split(/\n+/);
  for (const line of lines) {
