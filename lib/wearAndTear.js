    ...
    return visionIssues.map((issue) => {
      ...
      if (chargeTenant) {
        return {
          ...
          chargeTenant: true,
          answer: "Yes",
          reason: description || `New damage to ${itemLabel} beyond normal wear.`,
        };
      }

      const wearVerdict = getChargeVerdict(
        Boolean(description) || damageDetected,
        itemKey,
        tenancyYears,
        changePercent
      );
      return {
        itemLabel,
        itemKey,
        description: description || wearVerdict.reason,
        chargeTenant: false,
        answer: "No",
        reason: description
          ? `${itemLabel}: ${description} — normal wear for this tenancy.`
          : wearVerdict.reason,
      };
    });
  }
